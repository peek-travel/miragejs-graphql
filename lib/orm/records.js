...
export function getRecords(type, args, mirageSchema) {
  let collectionName = mirageSchema.toCollectionName(type.name);
  // for relay-pagination and interfaces we need the list of collection to 
  // search from, for now they are passed in the args as collectionNames and
  // the results concatenated.
  if (args.collectionNames) {
    const collectionNames = [...args.collectionNames];
    delete args.collectionNames;

    let allRecords = [];
    collectionNames.forEach((collectionName) => {
      let records = mirageSchema[collectionName].where(args).models;
      allRecords = allRecords.concat(records);
    });

    return adaptRecords(allRecords);
  }
